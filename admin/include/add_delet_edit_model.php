          ...
          <div class="modal-content bg-info">
            ...
            <div class="modal-body">
              ...
              <div class="card-header">
                <h3 class="card-title">DELET / EDITE ADMIN</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->';
              
               $quary ="SELECT * FROM admin WHERE admin_id={$_SESSION['ide']}";
                                      $result=mysqli_query($database->conn,$quary);
                                    $admin=mysqli_fetch_assoc($result);
              echo'<form action="maneg_admin.php" method="post" novalidate="novalidate" enctype="multipart/form-data">
                <div class="card-body">
                  <div class="form-group">
                    <label for="exampleInputEmail1" style="color:#2ba8e5"> Email </label>';
                   echo" <input type='email' name='email' class='form-control' id='exampleInputEmail1'  value='{$admin["admin_email"]}'>";
                  echo'</div>
                  <div class="form-group">
                    <label for="exampleInputPassword1" style="color:#2ba8e5"> Password</label>';
                   echo" <input type='password' name='password' class='form-control' id='exampleInputPassword1'  value='{$admin["admin_pass"]}'>";
                  echo'</div>
                  <div class="form-group">
                    <label for="exampleInputname1" style="color:#2ba8e5"> Name</label>';
                   echo" <input type='text' name='fname' class='form-control' id='exampleInputname1' value='{$admin["admin_name"]}'>";
                  echo'</div>';
                 echo" <div class='form-group'>
                                        <label for='cc-number' class='control-label mb-1' style='color:#2ba8e5'>admin img</label>
                                        <input id='cc-number' name='imgadmin' type='file' class='form-control cc-number identified visa'  data-val='true'
